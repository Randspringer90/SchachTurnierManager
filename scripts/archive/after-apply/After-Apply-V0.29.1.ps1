...
function Set-Version {
    ...
    $text = Read-Text $Path
    $next = $text.Replace('0.29.0', '0.29.1')
    if ($next -eq $text) {
        Write-Step "$Path ist bereits auf 0.29.1 oder enthielt keine 0.29.0-Version mehr"
        return
    }

    Write-Text $Path $next
    Write-Step "$Path auf 0.29.1 gesetzt"
}

function Insert-BeforeAppRenderReturn {
    param(
        [string]$Text,
        [string]$Block
    )

    $appIndex = $Text.IndexOf('function App() {')
    if ($appIndex -lt 0) {
        throw 'function App() wurde nicht gefunden.'
    }
    ...
    foreach ($marker in $markers) {
        $index = $Text.IndexOf($marker, $appIndex)
        if ($index -ge 0) {
            return $Text.Insert($index, $Block)
        }
    }

    throw 'Render-return-Anker innerhalb von function App() nicht gefunden.'
}
...
# v0.29.0 hatte die Helfer an einer falschen Stelle eingefuegt. Dadurch war das JSX-Panel vorhanden,
# aber Variablen wie correctionJournalStatusClass waren im Render-Scope nicht sichtbar.
$oldHelperPattern = '(?s)\r?\n  function correctionJournalItems\(\) \{.*?\r?\n  function openLatestRoundPrint\(\): void \{\r?\n.*?\r?\n  \}\r?\n'
$helperOccurrences = [regex]::Matches($main, $oldHelperPattern).Count
if ($helperOccurrences -gt 0) {
    $main = [regex]::Replace($main, $oldHelperPattern, "`r`n")
    Write-Step "Alte Korrekturjournal-Helfer entfernt: $helperOccurrences"
} else {
    Write-Step 'Keine alten Korrekturjournal-Helfer gefunden'
}
...
$main = Insert-BeforeAppRenderReturn -Text $main -Block $helperBlock
Write-Step 'Korrekturjournal-Helfer im App-Render-Scope neu eingefuegt'
...
if ($main.Contains('Korrektur- und Eingriffsübersicht')) {
    ...
    $main = Insert-PanelBeforeHeading -Text $main -Headings @('<h3>Auslosungsfreigabe</h3>', '<h3>Turnierleiter-Exportcenter</h3>', '<h3>Import / Export</h3>') -Panel $panelBlock
...
if ($changelog.Contains('0.29.1')) {
    Write-Step 'CHANGELOG.md enthält v0.29.1 bereits'
} else {
    $entry = @'
...
$handoffPath = 'docs/HANDOFF_0_29_1.md'
$handoff = @'
