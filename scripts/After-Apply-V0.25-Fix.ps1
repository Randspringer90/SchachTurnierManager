...
function Write-Step([string]$Message) { Write-Host "[v0.25.0-fix] $Message" }
...
function Replace-Version([string]$Path) {
    ...
    if ($updated -ne $text) {
        ...
        Write-Step "$Path ist bereits auf 0.25.0 oder enthielt keine 0.24.1-Version mehr"
    }
}
...
Ensure-Contains $mainPath 'Turnierleiter-Exportcenter' {
    ...
    # Robust statt fixer String-Anker: vor der bestehenden Import-/Export-Karte einfügen.
    $normalized = $text -replace "`r`n", "`n"
    $marker = '<h3>Import / Export</h3>'
    $markerIndex = $normalized.IndexOf($marker, [System.StringComparison]::Ordinal)
    if ($markerIndex -lt 0) {
        throw 'Anker fuer Turnierleiter-Exportcenter nicht gefunden: <h3>Import / Export</h3> fehlt.'
    }
    ...
    if ($articleIndex -lt 0) {
        throw 'Anker fuer Turnierleiter-Exportcenter nicht gefunden: Import-/Export-Article konnte nicht bestimmt werden.'
    }

    return $normalized.Substring(0, $articleIndex) + $exportCenterCard.TrimEnd() + "`n`n" + $normalized.Substring($articleIndex)
...
foreach ($path in @(
    ...
    'scripts/After-Apply-V0.25-Fix.ps1'
)) {
