...
function Invoke-Step {
    ...
    Write-Host "[v0.16.1] $Name..." -ForegroundColor Cyan
    & $Command
...
(Get-Content -LiteralPath $program -Raw).Replace('version = "0.15.0"', 'version = "0.16.1"') | Set-Content -LiteralPath $program -Encoding utf8NoBOM
(Get-Content -LiteralPath $packageJson -Raw).Replace('"version": "0.15.0"', '"version": "0.16.1"') | Set-Content -LiteralPath $packageJson -Encoding utf8NoBOM
(Get-Content -LiteralPath $packageLock -Raw).Replace('"version": "0.15.0"', '"version": "0.16.1"') | Set-Content -LiteralPath $packageLock -Encoding utf8NoBOM
(Get-Content -LiteralPath $mainTsx -Raw).Replace('Lokaler Turnierleiter · v0.15.0', 'Lokaler Turnierleiter · v0.16.1') | Set-Content -LiteralPath $mainTsx -Encoding utf8NoBOM

# Changelog ergänzen
$change = Get-Content -LiteralPath $changelog -Raw
if (-not $change.Contains('## 0.16.1 - CSV-Import bewusst bestätigen und Vorlagen')) {
    $entry = @"
# Changelog
...
foreach ($needle in @(
    ...
    if (-not $verifyMain.Contains($needle)) {
        throw "v0.16.1 UI-Patch unvollständig: $needle fehlt in main.tsx"
    }
}
...
Write-Host '[v0.16.1] Nachkontrolle abgeschlossen. Bitte danach git status prüfen und committen.' -ForegroundColor Green
git status --short
