...
public sealed class TournamentExportFormatter
{
    ...
    /// - Nur aktive Spieler (wie in <paramref name="standings"/> enthalten); zurückgezogene
    ///   Spieler sind noch nicht abgedeckt (eigener Folgeschritt, analog zum
    ///   Withdrawal-Fix aus STM-FACH-001).
    ...
    ///   passend zu heutiger Praxis und Issue #3.
    /// </summary>
        ...
        int lastRound)
    {
        ...
        var line = new TrfLineBuilder()
            .Set(1, "001")
            .Set(5, row.StartingRank.ToString(CultureInfo.InvariantCulture).PadLeft(4))
            ...
            .Set(81, row.Points.ToString("0.0", CultureInfo.InvariantCulture).PadLeft(4))
            .Set(86, row.Rank.ToString(CultureInfo.InvariantCulture).PadLeft(4));
        ...
        for (var roundNumber = 1; roundNumber <= lastRound; roundNumber++)
        {
            ...
            line.Set(basePosition, opponentRank is int rank ? rank.ToString(CultureInfo.InvariantCulture).PadLeft(4) : "    ");
    ...
    private static string Trf16FideNumberField(Player? player)
    {
        ...
        return string.IsNullOrWhiteSpace(id) ? new string(' ', 11) : Truncate(id, 11).PadLeft(11);
    }
    ...
    private static string SanitizeLine(string value) => value.Replace("\r", string.Empty).Replace("\n", " ");
    ...
    /// muessen bereits auf ihre Zielbreite (ge)trimmt/gepolstert sein.
    /// </summary>
