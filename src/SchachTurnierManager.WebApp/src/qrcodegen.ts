/*
 * QR Code generator (kompakter, abhängigkeitsfreier TypeScript-Port).
 *
 * Basiert auf der MIT-lizenzierten Referenzimplementierung
 * "QR Code generator library" von Project Nayuki
 * (https://www.nayuki.io/page/qr-code-generator-library).
 *
 * Bewusst lokal eingebettet: keine Laufzeit-Dependency, kein externer Dienst,
 * kein Netzwerkzugriff. Erzeugt aus einem Text ein QR-Modulraster, das die App
 * als reines SVG rendert. Nur Byte-Modus (UTF-8) – ausreichend für lokale URLs.
 */
...
class Builder {
  ...
  constructor(public readonly version: number, public readonly ecl: Ecl) {
  ...
  build(dataCodewords: number[]): boolean[][] {
    ...
    for (let mask = 0; mask < 8; mask++) {
      ...
      this.applyMask(mask); // rückgängig (XOR)
    }
    ...
    for (let y = 0; y < this.size; y++) {
      for (let x = 0; x < this.size; x++) {
        ...
        switch (mask) {
          ...
          default: throw new Error('Ungültige Maske');
        }
  ...
  private getPenaltyScore(): number {
    ...
    for (const row of modules) {
      dark += row.reduce((sum, cell) => sum + (cell ? 1 : 0), 0);
    }
  ...
  private finderPenaltyAddHistory(currentRunLength: number, runHistory: number[]): void {
    if (runHistory[0] === 0) {
      currentRunLength += this.size; // Rand als heller Streifen zählen
    }
    runHistory.pop();
...
function encodeBytesToCodewords(version: number, ecl: Ecl, bytes: number[]): number[] {
  // Bitbuffer: Byte-Modus-Indikator (0100), Längenfeld, Daten.
  const bits: number[] = [];
...
/**
 * Erzeugt einen QR-Code aus Text (Byte-Modus, UTF-8). Wählt automatisch die kleinste
...
export function encodeText(text: string, ecl: Ecl = Ecl.Medium): QrCode {
  ...
  for (; ; version++) {
    if (version > MAX_VERSION) {
      throw new Error('Text ist zu lang für einen QR-Code.');
    }
  ...
  const codewords = encodeBytesToCodewords(version, ecl, bytes);
  const builder = new Builder(version, ecl);
  const modules = builder.build(codewords);
  return {
    size: builder.size,
    modules,
    getModule(x: number, y: number): boolean {
