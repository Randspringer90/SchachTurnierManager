...
- Guards/Gates: `scripts/Test-AgentInstructionIntegrity.ps1` (auch CI), `Test-AgentSkillReadiness.ps1`, `Test-AgentSkillProposalSafety.ps1`, `Test-PromptInjectionDefense.ps1`, `Test-KnowledgePersistenceSafety.ps1`, `Test-NightlyReadiness.ps1`. Wissensmanagement: `docs/knowledge/**` (`docs/architecture/KNOWLEDGE_MANAGEMENT.md`); Agent-/Skill-Lernsignale beginnen ausschließlich als lokale, nicht aktivierende DRAFTs. Nightly-/Resume-Daten sind SHA-gebundene T3-Checkpoints unter `output/`; Resume plant nur und blockiert bei Drift. Die zentrale Registrierung bleibt bis zu einer getrennten Owner-Aktivierung `READY_FOR_ACTIVATION`.
